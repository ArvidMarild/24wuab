...
let fullSizeButtons = document.querySelectorAll(".full-size");
fullSizeButtons.forEach((button) => {
  button.addEventListener("click", showImg);
});

let overlayDivs = document.querySelectorAll(".overlay");
overlayDivs.forEach((overlay) => {
  overlay.addEventListener("click", showImg);
});

function showImg() {
  let overlay = this.parentElement.querySelector(".overlay");
